...
do
        ...
        Input=$(cat bam_paths.txt | sed -n ''$i'p' | awk '{print $2}')
        SampleName=$(cat bam_paths.txt | sed -n ''$i'p' | awk '{print $1}')
        echo $SampleName
       output=/data/kelley/projects/tarcreek/data/mapping/Xmac_bwa_subsamples_only/03_alignement_summary/$SampleName.txt
       echo $output
